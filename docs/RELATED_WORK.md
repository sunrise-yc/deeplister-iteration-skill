...
这份文档记录 `vibe-lens-skill` 应该从哪些相邻工具里学习，以及它和这些工具的刻意差异。
...
| 本机 `gstack-retro` / `gstack-diagram` / `baoyu-infographic` 等 Skill | 复盘、图表、信息图、设计稿和浏览器验证 | 作为工具箱使用，不作为产品边界 | 它们能帮助生成图或做复盘，但不能让 `vibe-lens` 变成大而全平台 |
...
| 旧名字像迭代管理工具 | 容易误导 Agent 去排优先级或安排任务 | 改名为 `vibe-lens`，强调观察和展示 |
